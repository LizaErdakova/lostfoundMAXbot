...
Проект разрабатывается командой для хакатона VK Education в экосистеме MAX. Цель — построить безопасный сервис поиска потерянных и найденных вещей (особый акцент на животных) с единой базой объявлений, автоматическим матчингом, картой и мини-приложением, которое удобно открывать прямо из бота.
...
│   │   ├── seed.js         # 20 тестовых объявлений (Москва)
...
DB_HOST=127.0.0.1   # при Docker = mysql
DB_PORT=3307        # при Docker = 3306
DB_USER=dev
DB_PASSWORD=dev
DB_NAME=lostfound
```

`client/.env` (создать при необходимости):

```
VITE_API_BASE=http://localhost:8080
YANDEX_MAPS_API_KEY=ваш_ключ (используется в index.html)
```
...
npm run seed            # наполняет 20 демо-объявлений
...
> Убедись, что `server/.env` содержит настройки для подключения к контейнеру MySQL: `DB_HOST=mysql`, `DB_PORT=3306`.

```powershell
cd C:\Users\User\Desktop\Desk\lostfound   # корень репозитория
...
1. Укажи `MAX_BOT_TOKEN` в `server/.env`.
2. Для локальных тестов используется **long polling** (`server/src/polling.js`). Он стартует автоматически вместе с `npm run dev`. Бот обрабатывает команды:
